...
namespace Celeste.Mod.GhostKevinball.Net {
    public class GhostNetServer : GameComponent {
        ...
        public bool FirstKevinballMatch = true; 
        ...
        public List<uint> randomLevels = new List<uint>(); 
        public uint PickRandomLevel()
        {
            ...
            {
                randomLevels = new List<uint>() { 0, 1, 2, 3 };
            }
        ...
        public void StartKevinball(GhostNetConnection con, GhostNetFrame frame)
        {
            ...
            else
            {
                ...
                while(!success)
                {
                    if(idx >= KevinballQueue.Count)
                    {
                        success = true; 
                    }
                    else
                    {
                        uint pid = KevinballQueue[idx];
                        if (SpectatingPlayers.Contains(pid))
                        {
                            idx++; 
                        }
                        else
                ...
                player2 = KevinballQueue[idx];
                KevinballQueue.RemoveAt(idx);
            }
        ...
        public void SetSpec(uint id)
        {
            ...
            if(SpectatingPlayers.Contains(id))
            {
                ...
                str = PlayerMap[id].Name + " is no longer spectating.";
            }
            else
            {
                SpectatingPlayers.Add(id);
                str = PlayerMap[id].Name + " is now spectating.";
            }
        ...
        public void EndKevinballMatch(uint winner, uint loser, uint wintype)
        {
            ...
            if (ShuffleMode > 0)
            {
                ...
                {
                    CurrentKevinballLevel = PickRandomLevel(); 
                    justShuffled = true;
                }
            }
